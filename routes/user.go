...
func User(route *gin.Engine, userController controller.UserController, jwtService config.JWTService) {
	...
	route.GET("/choose-role", middleware.AuthenticateCookies(jwtService), userController.ViewChooseRole)
	...
	route.GET("/pick-up", middleware.AuthenticateCookies(jwtService), middleware.OnlyAllow(constants.ENUM_ROLE_USER), userController.ViewFAQ)
}
